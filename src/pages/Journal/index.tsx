      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <div className="rounded-[2rem] border border-border bg-surface p-6 shadow-[0_24px_48px_rgba(0,0,0,0.18)]">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.4em] text-accent/70">Mi Diario de Campo</p>
              <h1 className="mt-3 text-3xl font-bold text-white">{totalEntries} avistamientos registrados</h1>
            </div>
            <button
              type="button"
              onClick={() => navigate('/identify')}
              className="rounded-3xl bg-cyan-500 px-5 py-3 text-sm font-semibold text-black transition hover:bg-cyan-400"
            >
              Ir a Identificar
            </button>
          </div>
        ...
        {loading ? (
          <div className="flex min-h-[320px] items-center justify-center rounded-[2rem] border border-border bg-surface">
            <div className="flex flex-col items-center gap-4">
              <div className="h-14 w-14 animate-spin rounded-full border-4 border-white/10 border-t-cyan-400" />
              <p className="text-lg text-textLight/80">Cargando tus registros...</p>
            </div>
          </div>
        ) : filteredEntries.length === 0 ? (
          <div className="rounded-[2rem] border border-border bg-surface p-10 text-center">
            <Compass className="mx-auto h-20 w-20 text-emerald-400" />
            <h2 className="mt-6 text-3xl font-bold text-white">Tu diario está vacío</h2>
            <p className="mt-3 max-w-2xl mx-auto text-textLight/80">
              Identifica especies con EcoScan, FloraID o FaunaID para guardar tus avistamientos aquí.
            </p>
            <button
              type="button"
              onClick={() => navigate('/identify')}
              className="mt-8 rounded-3xl bg-emerald-500 px-6 py-3 text-sm font-semibold text-black transition hover:bg-emerald-400"
            >
              Ir a Identificar
            </button>
          </div>
          ...
          <>
            <div className="rounded-[2rem] border border-border bg-surface p-6">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm uppercase tracking-[0.3em] text-textLight/70">Filtrar</p>
                  <div className="mt-4 flex flex-wrap gap-3">
                    {(['all', 'fauna', 'flora'] as const).map(filter => (
                      <button
                        ...
                        className={`rounded-full px-4 py-2 text-sm font-semibold transition ${activeFilter === filter ? 'bg-emerald-500 text-black' : 'bg-white/5 text-white hover:bg-white/10'}`}
                      >
                ...
                <div className="rounded-3xl bg-[#0f172a] p-4 text-sm text-textLight/80">
                  <p>Total de entradas: <span className="font-semibold text-white">{filteredEntries.length}</span></p>
                  <p className="mt-2">Especies únicas: <span className="font-semibold text-white">{uniqueSpeciesCount}</span></p>
                  <p className="mt-2">Último registro: <span className="font-semibold text-white">{mostRecentEntryDate}</span></p>
                </div>
              </div>
            </div>
            ...
            <div className="grid gap-6">
              {filteredEntries.map(entry => {
                const species = entry.speciesId ? getSpeciesById(entry.speciesId) : undefined
                return (
                  <div key={entry.id} className="rounded-[2rem] border border-border bg-[#111827] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.16)]">
                    <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                      <div className="flex-1">
                        <p className="text-sm uppercase tracking-[0.24em] text-accent/70">{formatSpanishDate(entry.date)}</p>
                        <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
                          <div>
                            <p className="text-xl font-bold text-white">{species?.commonName ?? 'Especie desconocida'}</p>
                            {species?.scientificName ? (
                              <p className="text-sm italic text-textLight/70">{species.scientificName}</p>
                            ) : null}
                          </div>
                          {species ? (
                        ...
                        {entry.location ? (
                          <div className="mt-4 flex items-center gap-2 text-textLight/80">
